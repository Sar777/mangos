...
SpellAuraProcResult Unit::HandleShareDamagePctAuraProc(Unit* pVictim, DamageInfo* damageInfo, Aura const* triggeredByAura, SpellEntry const * procSpell, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 /*cooldown*/)
{
    if (Unit *pCaster = triggeredByAura->GetCaster())
    {
        SpellEntry const *spellInfo = triggeredByAura->GetSpellProto();

        uint32 shareDamage = damageInfo->damage * (triggeredByAura->GetModifier()->m_amount / 100.0f);
